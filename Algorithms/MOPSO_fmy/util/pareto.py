...
def compare(fitness_curr, fitness_ref):
...
def judge(fitness_curr, fitness_data, cursor):
    # 判断当前是否为非劣解
    for i in range(len(fitness_data)):
        if i == cursor:
            continue
        # 如果数据集中存在一个解可以完全支配当前解，则当前解为劣解
        if compare(fitness_curr, fitness_data[i]):
            return False
    return True
...
class Pareto:
    ...
    def pareto(self):
        while self.hasNext():
            ...
            # 判断当前粒子是否pareto最优
            if judge(fitness_curr, self.fitness_data, self.cursor) == False:
                self.remove()
